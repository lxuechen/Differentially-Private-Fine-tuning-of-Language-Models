    ...
    def _build_optimizer(self):
        ...
        for m in self.model.modules():
            if(hasattr(m, 'in_proj')):
                params.append(m.in_proj.weight)
        ...
        print('shapes of parameters to update:')
        for p in params:
            print('paramter shape:', p.shape)
            if(self.args.sigma > 0):
                p.requires_grad = False # we will compute per-example gradients and assign the .grad attribute manually
            else:
        ...
        print('number of trainable parameters: ', num_p/1000, 'K')
            ...
            try:
                with maybe_no_sync():
                    ...
                    if(self.args.sigma > 0):
                        ...
                        if(classify_weight.grad == None): # clip and assign gradient
